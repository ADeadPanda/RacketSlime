...
public class OrangeSpawn implements CommandExecutor {
    private Main plugin = Main.getInstance();
    private Commands commands = plugin.commands;

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (command.getName().equalsIgnoreCase(commands.cmd7)) {
                ...
                if (player.hasPermission("RacketSlime.set.TeamSpawn")) {
                    if (args.length == 1) {
                        if (args[0].equalsIgnoreCase("spawn")) {
                            plugin.getConfig().set("OrangeSpawn", location);
                            plugin.saveConfig();
                            player.sendMessage(color.Set("&aConfig set for &6Orange &aSpawn."));
                        }
                    } else {
